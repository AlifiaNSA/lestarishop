...
const Collection = () => {
  ...
  const toggleFilter = (value, setState) => {
    setState((prev) => 
      prev.includes(value) ? prev.filter((item) => item !== 
      value) : [...prev, value]
    )
  }
  ...
  const applyFilter = () => {
    ...
    if(subCategory.length) {
      filtered = filtered.filter((product) => 
        subCategory.map(s => s.toLowerCase()).includes(product.subCategory.toLowerCase())
      )
    }
        ...
        <div className='min-w-72 bg-primary p-4 pt-8 pl-6 lg:pl-12 rounded-r-xl'>
          ...
          <div className='px-4 py-3 mt-6 bg-white rounded-xl'>
            ...
            <select onChange={(e) => setSortType(e.target.value)}
              className='border border-slate-900/5 outline-none text-gray-30 
              medium-14 h-8 w-full rounded px-2'
            >
